module MenusCommands
  ...
  INFO_MENU = {
    ...
    '6' => { label: 'Список вагонов поезда', func: :wagons_list },
    '0' => { label: 'Выход в главное меню', func: :change_menu }
  }.freeze
  MAIN_MENU = {
  ...
  def wagons_list
    ...
    train_index = ask_i('Укажите индекс поезда для списка вагонов:')
    train = @trains[train_index]
    return if incorrect?(train)
...
занято #{wagon.seats_occupied} мест"
  ...
  def train_load_wagon
    ...
    wagon_index = ask_i('Укажите индекс вагона для загрузки:')
    wagon = train.wags[wagon_index]
    return if incorrect?(wagon)
